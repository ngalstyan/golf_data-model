"""
Script 12 — Pull 2025 Season Data (SG Rounds + Schedule + Odds + Matchup Odds)

Pulls all PGA Tour data for 2025 from the DataGolf API:
  1. Round-level SG data → data/raw/sg_rounds_pga_2025.csv
  2. Schedule/events     → data/raw/schedule_2025.csv
  3. Outright odds       → data/raw/odds_2025.csv
  4. Matchup odds        → data/raw/matchup_odds_2025.csv

Rate limit: 45 requests/minute → 1.5s delay between requests.
Supports --resume to skip already-fetched events (for matchup/odds).

Run: python 12_pull_2025_data.py --key YOUR_API_KEY
     python 12_pull_2025_data.py --key YOUR_API_KEY --resume
     python 12_pull_2025_data.py --key YOUR_API_KEY --only rounds
     python 12_pull_2025_data.py --key YOUR_API_KEY --only matchups
"""
...
def pull_sg_rounds(api_key: str):
    print("\n" + "=" * 60)
    print("STEP 1: SG Rounds (PGA 2025)")
    print("=" * 60)

    out_path = OUT_DIR / "sg_rounds_pga_2025.csv"
    total_rows = 0
    fieldnames = None
    csv_file = None
    writer = None
...
def pull_schedule(api_key: str):
    print("\n" + "=" * 60)
    print("STEP 2: Schedule/Events (2025)")
    print("=" * 60)

    out_path = OUT_DIR / "schedule_2025.csv"
...
def pull_odds(api_key: str, resume: bool = False):
    print("\n" + "=" * 60)
    print("STEP 3: Outright Odds (2025)")
    print("=" * 60)

    out_path = OUT_DIR / "odds_2025.csv"
    ...
    print(f"{len(events_2025)} events in 2025")

    if not events_2025:
        print("  No 2025 events found in odds system yet.")
        return
...
def pull_matchup_odds(api_key: str, resume: bool = False):
    print("\n" + "=" * 60)
    print("STEP 4: Matchup Odds (2025)")
    print("=" * 60)

    out_path = OUT_DIR / "matchup_odds_2025.csv"
    ...
    with open(out_path, write_mode, newline="", encoding="utf-8") as f:
        ...
        for i, e in enumerate(events_2025):
            ...
            for book in ["pinnacle"]:  # Pinnacle only for matchups
                ...
                print(f"\n  [{i+1}/{len(events_2025)}] {event_name} ({cal_year}) / {book}...",
                      end=" ", flush=True)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pull 2025 PGA data from DataGolf API")
    ...
    print("=" * 60)
    print("Script 12 — Pull 2025 PGA Season Data")
